...
export const Cell = memo(function Cell({
  value, notes, isGiven, isLocked, isSelected, isRelated, isSameNumber, isError, row, col, onClick
}: Props) {
  ...
  return (
    ...
    >
      {value !== 0 ? (
        <span className={`font-mono text-[clamp(14px,3.5vw,26px)] leading-none ${textColor} ${isSelected ? 'animate-pop' : ''}`}>
          {value}
        </span>
      ) : notes.size > 0 ? (
